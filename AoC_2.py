class AoC_2:
	...
	spreadsheet  = [["116     1259    1045    679     1334    157     277     1217    218     641     1089    136     247     1195    239     834"],
	...
	int_spreadsheet = [list(map(int, list(filter(None, x[0].split(' '))))) for x in spreadsheet]
	checksum = sum([max(x) - min(x) for x in int_spreadsheet])
	print("part 1 : " + str(checksum))

	for s in int_spreadsheet:
		for x in s:
			for i in range(len(s)):
				if x % s[i] == 0 and x != s[i]:
					checksum.append(x / s[i])
					break
